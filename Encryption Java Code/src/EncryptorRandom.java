...
public class EncryptorRandom {
    ...
    // Encrypt a message with AES and apply a random character shift
    public static String encrypt(String message, SecretKey secretKey) throws Exception {
        ...
        int shift = new Random().nextInt(10) + 1; // Random shift between 1 and 10
        String shiftedMessage = shiftString(encryptedMessage, shift);

        // Store the shift value as part of the output, separated by a colon
        return shift + ":" + shiftedMessage;
    }

    // Decrypt a message with AES after reversing the random character shift
    public static String decrypt(String shiftedMessageWithKey, SecretKey secretKey) throws Exception {
        // Split the stored shift value and the actual encrypted message
        String[] parts = shiftedMessageWithKey.split(":", 2);
        int shift = Integer.parseInt(parts[0]);
        String shiftedMessage = parts[1];

        // Reverse shift each character by the shift value before decryption
        String encryptedMessage = shiftString(shiftedMessage, -shift);
